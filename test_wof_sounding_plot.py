...
#Figure User Input
   ...
   my_dpi = 60 #dots per inch for the plot. This is a pretty hi-res image.
...
# Get the Bunkers storm motions and convert them to strings to plot
   ...
   if all(bunkers==np.nan): bunkers[:]=0.0
...
# Write the parcel properties to the inset.
   x_list = np.array([0, 0.07, 0.15, 0.22, 0.29, 0.34, 0.41, 0.48])-0.075
   y_list = [-0.045, -0.07, -0.0925, -0.115, -0.1375]
   A = ["SFC", prof.sfcpcl.bplus, int(prof.sfcpcl.bminus), prof.sfcpcl.lclhght,prof.sfcpcl.li5, prof.sfcpcl.lfchght, prof.sfcpcl.elhght, prof.pw*25.4]
#   B = ["FCST", prof.fcstpcl.bplus, int(prof.fcstpcl.bminus), prof.fcstpcl.lclhght, prof.fcstpcl.li5, prof.fcstpcl.lfchght, prof.fcstpcl.elhght]
   C = ["ML", prof.mlpcl.bplus, int(prof.mlpcl.bminus), prof.mlpcl.lclhght, prof.mlpcl.li5, prof.mlpcl.lfchght, prof.mlpcl.elhght, prof.pw*25.4]
   D = ["MU", prof.mupcl.bplus, int(prof.mupcl.bminus), prof.mupcl.lclhght, prof.mupcl.li5, prof.mupcl.lfchght, prof.mupcl.elhght, prof.pw*25.4]
   #mlcape = C[1]
   #print('mlcape',mlcape)
   data = np.array([["PCL", "CAPE", "CINH", "LCL", "LI", "LFC", "EL", "PW"],
...
# Draw the CAPE vs. SRH Scatter
   ...
   #for member_no,c in zip(np.arange(1), np.tile([cb_colors.orange6, cb_colors.orange6, cb_colors.green6, cb_colors.green6, cb_colors.purple6, cb_colors.purple6], (3,1))):
