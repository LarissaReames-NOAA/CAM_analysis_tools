    ...
    if nthreads < 0:  # turning this off for now.
        def worker(i):
            ...
            for j in np.arange(data.shape[1]):
                  dinterp[:,j,i] = log_interp(p1d[::-1], p3d[:,j,i], data[:,j,i])
        ...
        for i in np.arange(data.shape[2]):
            for j in np.arange(data.shape[1]):
                dinterp[:,j,i] = log_interp(p1d[::-1], p3d[:,j,i], data[:,j,i])
        print(np.amax(dinterp[-1,:,:]), np.amin(dinterp[-1,:,:]))
        return dinterp[::-1,:,:]
...
def hrrr_grib_read_variable(file, sw_corner=None, ne_corner=None, var_list=[''], interpP=True, writeout=True, prefix=None, isIsobaric = False):
    
    # Special thanks to Scott Ellis of DOE for sharing codes for reading grib2
    
    default = {            #  Grib2 name                 / No. of Dims /  Type   / bottomLevel / paramCategory / paramNumber
               'TEMP':     ['Temperature',                           3, 'isobaricInhPa'],
               'OMEGA':    ['Vertical velocity',                     3, 'isobaricInhPa'],
               'U':        ['U component of wind',                   3, 'isobaricInhPa'],
               'V':        ['V component of wind',                   3, 'isobaricInhPa'],
               #'UH':       [1018,                                    2, 'hybrid'],
               #'CREF':     [1008,                                    2, 'hybrid'],
               'UH':       ['199',                                    2, 'isobaricInhPa', 2000,         7,              199],
    ...
    if isIsobaric:
        level_type_grib = 100
        level_type_string = 'isobaricInhPa'
    else:
        level_type_grib = 105
    ...
    grb_file = pygrib.open(file)#[593]
    #for key in grb_file.keys():
    #    print(grb_file[key])
    #for grb in grb_file:
    #    print((grb.typeOfLevel,grb.name, grb.level))
    ...
    if (isIsobaric):
        grbs = grb_file.select(name='Temperature', typeOfLevel=level_type_string)
        nz = len(grbs)
        p3d = np.full((nz,ny,nx),np.nan)
        levels = np.array([grb_element['level'] for grb_element in grbs])
        indexes    = np.argsort(levels)
        levels = levels[indexes]
        for i in np.arange(nz):
            p3d[i,:,:] = np.broadcast_to(levels[i],[ny,nx])*100.0
    else:
        ...
        if variables[key][1] == 3:
            ...
            if interpP:
                ...
                new = xr.DataArray( cube['data'], dims = ['nz','ny','nx'], 
                                                  coords={"lats": (["ny","nx"], lats),
                                                          "lons": (["ny","nx"], lons), 
                                                          level_type_string: (["nz"],     cube['levels']) } )
    ...
    if interpP : 
        ds_conus['W'] = xr.DataArray( w_new, dims = ['nz','ny','nx'], 
                                      coords={"lats": (["ny","nx"], lats),
                                              "lons": (["ny","nx"], lons), 
                                              "pres": (["nz"],      plevels) } )
        ds_conus['P'] = xr.DataArray( pp3, dims = ['nz','ny','nx'], 
                                      coords={"lats": (["ny","nx"], lats),
                                              "lons": (["ny","nx"], lons), 
                                              "pres": (["nz"],      plevels) } )   
    else:
...
def fv3_grib_read_variable(file, sw_corner=None, ne_corner=None, var_list=[''], writeout=True, prefix=None):
    ...
    pp3 = np.broadcast_to(pres, (nx,ny,len(pres))).transpose()
